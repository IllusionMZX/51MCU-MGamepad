...
/**
  ...
  */
uint8_t OLED_pnpoly(uint8_t nvert, int* vertx, int* verty, int testx, int testy)
{
    int i, j, c = 0;
...
/**
  ...
  */
uint8_t OLED_IsInAngle(int X, int Y, int StartAngle, int EndAngle)
{
    int PointAngle;
...
void OLED_ShowFloatNum(int X, int Y, double Number, uint8_t IntLength, uint8_t FraLength, uint8_t FontSize)
{
    ...
    FraNum = (unsigned long)(Number * PowNum + 0.5);		//将小数乘到整数，同时四舍五入，避免显示误差
...
void delay_ms(unsigned int time) //@27MHz 毫秒延时
{
    unsigned int y;
    for(y = 0; y < time; y++) {
        unsigned char i, j;
...
void OLED_DrawRestart()
{
    ...
    //OLED_ShowString(10, 3, "GAME", OLED_6X8);
    //OLED_ShowString(86, 3, "OVER", OLED_6X8);
}
